...
| Transaction executor      | Apply an already validated plan through a narrow sequential or batch adapter, verify observed result state, classify exact failure progress, and compensate to the verified initial snapshot after failure. | Implemented in Milestone 1 and refactored into shared sequential/batch machinery in Milestone 4.            |
| Studio MCP adapter        | Select an exact sandbox, verify engine state, and translate the allowlisted adapter protocol to fixed single or chunked Studio operations.                                                                  | Milestone 3 local-stdio safety boundary plus Milestone 4 chunking and reconnectable recovery.               |
