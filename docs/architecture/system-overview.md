...
The transaction layer includes only an in-memory test adapter. No live Roblox Studio adapter, Forge
plugin, Studio MCP transport, AI provider, or actual Roblox world mutation is implemented.
...
flowchart LR
    ...
    class WorldSpec,ArchitecturePlanner,RobloxCompiler current;
    class Inputs,Atlas,FuturePlanners,Router,LiveAdapter,World,Critic,Repair future;
```
...
[Roblox compiler and transaction architecture](roblox-compiler.md) for their respective data flow,
trust boundaries, hashing, and safety rules.
...
7. Through an abstract adapter, reject stale state before mutation, apply operations sequentially,
...
exposes offline `compile` and change-set `plan`. Neither exposes a live `apply` command.
