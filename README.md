...
> **Status:** Milestone 0, the WorldSpec v0.1 foundation, is complete. Milestone 1 is the current
> implementation: a deterministic Roblox primitive compiler, dry-run reconciler, and verified
> transaction protocol. There is no live Roblox Studio integration.
...
ChangeHistoryService integration, or CLI apply command. Atlas orchestration, spatial planning,
constraint solving, asset routing or generation, The Critic, and the Reference-to-Mansion vertical
slice remain future work.
...
2. **Transactional Roblox primitive compiler** - deterministic desired manifests, observed
   ...
   current implementation; offline only).
...
Roadmap items after the offline transactional compiler describe direction, not current capability.
