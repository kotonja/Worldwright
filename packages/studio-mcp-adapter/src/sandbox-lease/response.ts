...
import type { StudioSandboxLeaseRequest, StudioSandboxLeaseResponse } from './types.js';
import { validateStudioSandboxLeaseResponseForRequest } from './validate.js';
...
export function parseStudioSandboxLeaseResponse(
  text: string,
  expectedRequest: Readonly<StudioSandboxLeaseRequest>,
  ...
  if (byteLength > STUDIO_MCP_MAX_RESULT_BYTES || byteLength > STUDIO_MCP_MAX_BRIDGE_TEXT_BYTES) {
    throw new StudioAdapterError([
      studioDiagnostic(
        ...
        '',
        'Studio sandbox lease response exceeds the bounded bridge result size.',
      ),
    ]);
  }
  const prefixIndex = text.indexOf(STUDIO_SANDBOX_LEASE_RESPONSE_PREFIX);
  const secondPrefixIndex = text.indexOf(
    STUDIO_SANDBOX_LEASE_RESPONSE_PREFIX,
    prefixIndex < 0 ? 0 : prefixIndex + STUDIO_SANDBOX_LEASE_RESPONSE_PREFIX.length,
  );
  if (prefixIndex !== 0 || secondPrefixIndex !== -1 || !text.endsWith('\n')) {
    responseInvalid('Studio sandbox lease response has invalid or ambiguous framing.');
  }
  const encoded = text.slice(STUDIO_SANDBOX_LEASE_RESPONSE_PREFIX.length, -1);
  if (encoded.length === 0 || encoded.trim() !== encoded) {
    responseInvalid('Studio sandbox lease response contains noncanonical framing output.');
  }
    ...
    if (/(?:\.\.\.|…)[ \t]*\(truncated\)[ \t\r\n]*$/iu.test(encoded)) {
      throw new StudioAdapterError([
        studioDiagnostic(
          'studio.response_too_large',
          '',
          'Studio sandbox lease response was truncated by the MCP transport.',
        ),
      ]);
    }
    responseInvalid('Studio sandbox lease bridge returned malformed JSON.');
  }
  if (containsDuplicateObjectKeys(encoded)) {
    responseInvalid('Studio sandbox lease response contains duplicate object keys.');
  }
  const validation = validateStudioSandboxLeaseResponseForRequest(parsed, expectedRequest);
